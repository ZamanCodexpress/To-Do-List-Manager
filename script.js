...
const dailyTasks = {
  ...
  addTask(taskName) {
    if (state.dailyTasks.length >= this.MAX_TASKS) {
      alert(
        `You can only have ${this.MAX_TASKS} daily tasks. Please remove one to add a new task.`,
      );
      return;
    }
  ...
  render() {
    ...
    if (state.dailyTasks.length === 0) {
      container.innerHTML =
        '<div class="empty-daily-tasks">No daily tasks yet. Add your first one above!</div>';
      this.updateProgress();
      return;
    }
        ...
        datasets: [
          {
            ...
            borderColor: ["rgba(0, 212, 255, 1)", "rgba(99, 102, 241, 1)"],
            borderWidth: 2,
          },
        ],
      },
